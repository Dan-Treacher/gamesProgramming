...
function LevelMaker.createMap(level)
    ...
    -- Called once per level initialisation and it flags whether a locked brick is in play (want this as a global variable)
    --lockedBrick = math.random(1, 2) == 1 and true or false -- Ternary statement to map 1 or 2 to true or false
    lockedBrick = true -- With this active, there is always a locked brick included
    if lockedBrick == true then
        insertLockedBrick = true  -- This is used to activate a statement within the level build loop ONCE
    end
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            -- It's brick.color and brick.tier that dictates which one of the sprites is rendered for the brick
            local r1 = math.random(1,5) == 1 and true or false
            if (insertLockedBrick == true) and (r1 == true) then
                b.locked = 1
                insertLockedBrick = false  -- Ensures this triggers only once in the level creation
            end
