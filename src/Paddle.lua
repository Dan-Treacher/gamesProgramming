...
function Paddle:render()
    ...
    -- Not only do you render the texture of the appropriate size and skin colour, but you check its size and update the paddle hitbox
    local w
    _, _, w, _ = gFrames['paddles'][self.size + 4 * (self.skin - 1)]:getViewport()
    self.width = w
    -- In above lines, :getTextureDimension() doesn't work, but :getViewport() does for whatever reason
end